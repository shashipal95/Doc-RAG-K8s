...
def extract_text_from_file(file: UploadFile) -> str:
    ...
    if file.filename.endswith(".txt"):
        ...
        raise HTTPException(
            status_code=400, detail="Unsupported file type. Use .txt, .pdf, or .docx"
        )
...
def get_embedding(text: str) -> List[float]:
    ...
    try:
        # Sanitize text: Remove null bytes and 'unknown encoding' artifacts
        clean_text = (
            text.replace("unknown encoding:", "")
            .encode("ascii", "ignore")
            .decode("utf-8")
        )
...
def get_query_embedding(text: str) -> List[float]:
    """Generate query embedding using Google GenAI SDK"""
    try:
        result = client.models.embed_content(
            model=model,
            contents=text,
...
async def upload_file(file: UploadFile = File(...)):
    ...
    try:
        ...
        if not chunks:
            raise HTTPException(
                status_code=400, detail="No valid chunks created from file"
            )
        ...
        for i, chunk in enumerate(chunks):
            ...
            metadata = {"text": chunk, "filename": file.filename, "chunk_index": i}
...
async def query_documents(request: QueryRequest):
    ...
    try:
        ...
        # Search Pinecone
        results = index.query(
            vector=query_embedding, top_k=request.top_k, include_metadata=True
        )

        if not results["matches"]:
            return QueryResponse(
                answer="No relevant information found in the database.", sources=[]
            )
